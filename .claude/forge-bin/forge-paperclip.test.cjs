...
// ===================================================================================
{
  ...
  const sample = {
    ...
    base: 'http://127.0.0.1:3100', pcHome: 'C:/Users/YOUR_USERNAME/paperclip-home', updated: '2026-07-14T00:00:00.000Z',
...
// ===================================================================================
{
  const winPath = 'C:\\Users\\YOUR_USERNAME\\.local\\bin\\claude.exe';
  const out = pc.toForwardSlashes(winPath);
  t('F1 a Windows-style path is fully converted to forward slashes', !out.includes('\\'));
  t('F1 the path segments themselves are preserved (no data loss in the transform)', out === 'C:/Users/YOUR_USERNAME/.local/bin/claude.exe');
...
// ===================================================================================
{
  ...
  t('H3 no env override, lookup throws (not found on PATH) -> hardcoded last resort', pc.resolveClaudeBin({ env: {}, lookup: () => { throw new Error('not found'); } }) === 'C:/Users/YOUR_USERNAME/.local/bin/claude.exe');
  t('H4 no env override, lookup returns an empty string -> hardcoded last resort', pc.resolveClaudeBin({ env: {}, lookup: () => '' }) === 'C:/Users/YOUR_USERNAME/.local/bin/claude.exe');
