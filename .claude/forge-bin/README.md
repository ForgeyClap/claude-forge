...
| `forge-gate-hook.cjs` | A `PreToolUse` hook (matcher `Bash`/`PowerShell`) that turns the 4 COMMAND hard gates (`destructive-delete`, `kill-by-name`, `git-destructive`, `opaque-exec`) into a real stop — exit 2 blocks the tool call and explains why (NL+EN) instead of just advising against it. A classifier, not a proof: what it does not recognise it does not stop. Controlled by config key `gate-hook` (default ON — only the owner turns it off). **Not started by any wrapper above** — it only runs when wired into `.claude/settings.json` as a hook (see `config/orchestration/HOOKS_OPT_IN.md`). |
| `forge-settings-merge.cjs` | `apply`/`check` — merges Forge's `.claude/settings.json` (every hook + `permissions.deny` rule) into an EXISTING project's own settings.json instead of leaving it for the owner to merge by hand: foreign hooks/rules/keys are kept in place (values untouched, order preserved, BOM/EOL/indent preserved) when the file can be reserialized losslessly; a file that cannot (duplicate keys, unsafe numbers) is left alone and reported; a verified-missing regular file -> created exclusively, present -> merged with a unique `.forge-bak-<ts>` backup first and re-verified right before the rename, malformed/unexpected shape or a directory-shaped target -> refused-safe (`settings.forge-recommended.json` written instead); every destination must lie inside the real project root (guards in `forge-settings-merge-guards.cjs`, codex-recheck 2026-09-24). Called by `install.sh`/`install.ps1` and by `forge-sync.cjs`'s own `syncProjectSettings()` — see `config/orchestration/HOOKS_OPT_IN.md`. |
