...
const T = {
  nl: {
    ...
    scopeStrengthened: (k) => 'Let op: ' + k + ' geldt voor de hele computer, maar de waarde in het projectbestand zet het terug naar de veilige standaard — dat mag altijd (een project kan een globale bescherming alleen versterken, nooit verzwakken).',
    globalIgnoredForSafety: (k) => 'Let op: ' + k + ' staat (ook) in het globale bestand, maar wordt daar nooit uit gelezen — deze instelling geldt bewust alleen per project, zodat een reset nooit een verborgen globale "uit" tevoorschijn kan halen.',
    ...
    onceAlreadyArmed: (k) => 'Er staat al een eenmalige toestemming voor ' + k + ' klaar die nog niet is gebruikt — maak dat ene commando eerst af, of wacht tot die vanzelf verloopt, voordat je een nieuwe aanvraagt.',
    oncePendingRefuse: (k) => 'Er staat een eenmalige toestemming voor ' + k + ' klaar die nog niet is gebruikt of verlopen — dit gewone commando wordt geweigerd, zodat die toestemming nooit stilzwijgend permanent wordt. Maak dat ene commando eerst af (het gaat vanzelf weer aan), wacht tot de toestemming verloopt, of zet ' + k + ' nu meteen aan met: /forge config set ' + k + ' aan',
    globalIgnoredRefuse: (k) => k + ' wordt nooit uit het globale bestand gelezen (bewust alleen per project) — een globale waarde zou dus nooit iets doen. Zet het per project: /forge config set ' + k + ' <waarde>',
  ...
  en: {
    ...
    scopeStrengthened: (k) => 'Note: ' + k + ' is machine-wide, but the project file\'s value puts it back at its safe default — that is always allowed (a project can only STRENGTHEN a global protection, never weaken it).',
    globalIgnoredForSafety: (k) => 'Note: ' + k + ' also has a value in the global file, but it is never read from there — this setting is deliberately project-only, so a reset can never expose a hidden global "off".',
    ...
    onceAlreadyArmed: (k) => 'A one-off approval for ' + k + ' is already armed and not yet used — finish that one command first, or wait for it to expire, before asking for a new one.',
    oncePendingRefuse: (k) => 'A one-off approval for ' + k + ' is armed and not yet used or expired — this ordinary command is refused, so that approval can never silently become permanent. Finish that one command first (it switches back on by itself), wait for the approval to expire, or turn ' + k + ' on right now with: /forge config set ' + k + ' on',
    globalIgnoredRefuse: (k) => k + ' is never read from the global file (deliberately project-only) — a global value would never do anything. Set it per project instead: /forge config set ' + k + ' <value>',
