#!/usr/bin/env node
'use strict';
/** Tests for the 12 Piece-I domain playbooks (forge-agent, forge-api, forge-bots, forge-cli, forge-cms,
 *  forge-data, forge-extension, forge-figma, forge-game, forge-migration, forge-mlops, forge-mobile): each
 *  SKILL.md exists, has valid+matching frontmatter, and carries every required playbook section (the same
 *  shape as the already-proven forge-website/forge-payments playbooks); each has a matching
 *  config/rubrics/<domain>.json rubric with real criteria; and forge-router/SKILL.md actually references
 *  each playbook (Step 1 domain table) so a build can never silently miss the routing row. Mirrors
 *  forge-playbooks.test.cjs's shape (the Wave-E1 4-playbook batch) for this 12-playbook batch: real
 *  assertions against the live template PLUS a hermetic mutation fixture proving the section-lint actually
 *  bites. Never writes into the real template. */
const fs = require('fs');
...
// domain slug -> {skillDir: forge-<slug>, rubricFile: config/rubrics/<slug>.json}. Rubric filenames
// intentionally drop the "forge-" prefix — matches the real files on disk (same convention as the
// Wave-E1 batch: payments.json, not forge-payments.json).
const PLAYBOOKS = ['agent', 'api', 'bots', 'cli', 'cms', 'data', 'extension', 'figma', 'game', 'migration', 'mlops', 'mobile'];

// Required playbook sections — the exact shape every proven playbook already carries. Matched as a
// line-start prefix so minor heading suffixes ("(non-negotiable)", "(conditional)", etc.) don't break the
// check — only a genuinely MISSING section does.
...
// against the real 12 playbooks below AND against a hermetic mutated fixture (mutation-verify requirement).
function lintPlaybook(text) {
  const missing = REQUIRED_SECTIONS.filter((s) => !s.re.test(text)).map((s) => s.label);
  return { ok: missing.length === 0, missing };
}

// ---- REAL: the 12 live playbook SKILL.md files ----
...
// ---- REAL: the router references every playbook (Step 1 domain table) ----
...
if (routerExists) {
  ...
  for (const domain of PLAYBOOKS) {
    ...
    t('forge-router references `forge-' + domain + '` (Step 1 routing row present)', routerText.includes(needle));
  }
}
...
const REAL_AGENT_TEXT = fs.readFileSync(path.join(SKILLS_DIR, 'forge-agent', 'SKILL.md'), 'utf8');

// clean baseline: the real file must lint clean
const cleanLint = lintPlaybook(REAL_AGENT_TEXT);
t('hermetic baseline: real forge-agent/SKILL.md lints clean (0 missing sections)', cleanLint.ok === true && cleanLint.missing.length === 0);

// MUTATION 1: strip the "## Domain gates" section header (rename it) -> lint must catch it
const mutatedNoDomainGates = REAL_AGENT_TEXT.replace(/^## Domain gates/m, '## Renamed Section');
...
const mutatedNoShipReadiness = REAL_AGENT_TEXT.replace(/^## Ship-readiness[\s\S]*?(?=^## |\s*$)/m, '');
...
const restoredLint = lintPlaybook(fs.readFileSync(path.join(SKILLS_DIR, 'forge-agent', 'SKILL.md'), 'utf8'));
t('hermetic: real forge-agent/SKILL.md on disk is untouched after mutation checks (still lints clean)', restoredLint.ok === true);
...
const TMP = fs.mkdtempSync(path.join(os.tmpdir(), 'forge-playbooks-i-'));
