...
<!--
  ...
  License: MIT (c) 2025 Siqi Chen
  Adaptation: AskUserQuestion removed from allowed-tools for non-interactive Forge use (see note below).
  Reviewed for prompt-injection patterns and hidden/zero-width/control unicode by build-boss, 2026-07-12 -- clean.
-->
...
Note (Forge adaptation): AskUserQuestion was removed from allowed-tools for non-interactive Forge Boss runs — voice/style guidance and any writing sample must be passed upfront in the task context, not requested interactively mid-run.
