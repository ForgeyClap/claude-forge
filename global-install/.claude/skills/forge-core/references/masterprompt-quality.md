...
- **Claude / Claude Code (Opus 4.8 default):** be explicit + literal; front-load intent/scope/constraints/acceptance in turn 1; add "only make changes directly requested; don't over-engineer"; scope to exact files/paths; **mandatory stop conditions + human-review triggers**; do NOT hardcode effort/thinking budget (harness-managed).
...
*Route by **alias** — `opus`/`sonnet`/`haiku`/`fable` — which is version-proof and is exactly what the Agent tool accepts. Current display names: Haiku 4.5 · Sonnet 5 · Opus 4.8 · Fable 5. Defer version specifics to the live `/model` picker.*
- **`opus` (Opus 4.8) — reserve for the genuinely hard / high-stakes.** Official: *"deep research and analysis you'll question, redirect, and build on"*; complex specialized, advanced reasoning; NOT for simple/quick (wastes rate limit). Forge uses it for: the **Lead/orchestration**, architecture, contract-first design, security/auth review, database migrations, hard debug (after a first Sonnet attempt), final integration verdict / adversarial review, production gate.
