...
test('buildMission derives real wps/tasks/decisions/verdicts from this run\'s own events', () => {
...
test('a real dispatch with TWO subagent_completed events (T1.7 slice) merges into one task with both notes', () => {
  const mission = buildMission(PROJECT_ROOT, RUN_ID);
...
test('A2 honesty: the literal orphan completion (a real dispatch_id explained in free text) surfaces as an orphan, not a fabricated task', () => {
  const mission = buildMission(PROJECT_ROOT, RUN_ID);
...
test('the WP3 task (this very work package, now genuinely completed) reflects its real final state', () => {
...
test('collapsed-id ambiguity is honestly flagged at lower confidence (role "cc-t17-slice" -> WP1, not WP17)', () => {
  const mission = buildMission(PROJECT_ROOT, RUN_ID);
