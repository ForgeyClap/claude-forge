...
test('#9 SECURITY: a bare "/%" (malformed percent-escape) returns 400, never crashes the process', async () => {
  const res = await request(port, '/%');
  assert.equal(res.statusCode, 400);
});

test('#9 SECURITY: the gateway is still alive and serving after the malformed-path request', async () => {
...
test('#9 SECURITY: other malformed-escape shapes (double-encoded, lone %, %-at-end) all degrade to 400, never throw', async () => {
...
test('#10 SECURITY: a real built asset (not just the index shell) carries the same security headers', async () => {
...
test('#10 SECURITY: the 400 "bad path" response also carries the security headers (defense in depth)', async () => {
  const res = await request(port, '/%');
  assert.equal(res.statusCode, 400);
  assert.equal(res.headers['x-frame-options'], 'DENY');
  assert.equal(res.headers['x-content-type-options'], 'nosniff');
...
test('AUTH: GET / carries a <meta name="cc-exec-token"> tag with the REAL current-boot token', async () => {
  const res = await request(port, '/');
  assert.equal(res.statusCode, 200);
...
test('AUTH: the exec-token meta tag is only injected into HTML, never into a real built asset', async () => {
  const indexRes = await request(port, '/');
  const assetMatch = indexRes.body.match(/(?:src|href)="\.?(\/assets\/[^"]+)"/);
  assert.ok(assetMatch, 'the built index.html must reference at least one /assets/ file to make this test meaningful');
  const assetRes = await request(port, assetMatch[1]);
