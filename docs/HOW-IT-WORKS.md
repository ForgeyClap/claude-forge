...
- **Usage guard.** Forge pauses at 98 % of your 5-hour or weekly limit, between steps, so a build is never cut off halfway; Claude Code itself continues after the reset. → [TOKEN-USAGE.md](TOKEN-USAGE.md)
- **Safety stop.** A hook blocks mass deletes, killing programs by name and git commands that throw away uncommitted work, and Claude cannot read your `.env` secret files. → [FEATURES.md](FEATURES.md#the-safety-stop-gate-hook--secret-deny-rules)
