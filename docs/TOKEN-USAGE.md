...
- **`usage-guard.cjs`** — a real subscription watchdog. It reads the **official Anthropic usage endpoint** (the same source as `/usage` — no estimates). When you hit **≥ 95%** of your 5-hour session window *or* your weekly window, it **pauses** the agents (the dashboard stays up), tells your session to pause, and **auto-resumes after the reset**. On any error it does nothing (fail-safe), and it never logs your token.
...
- **Use `/clear` between unrelated tasks** so you're not paying to re-read old context.
...
- *(Advanced, optional)* **Offload heavy build work to NVIDIA models.** Forge can call NVIDIA NIM models via `nvidia-provider.cjs` for the bulk implementation while your Lead stays on Claude — useful for large jobs where you want to spare Claude quota. This is opt-in and off by default; a mock is never presented as a real result, and keys live only in `.env` / a global key file (never in code).
...
- It **stops before you blow your quota** and resumes after the reset.
