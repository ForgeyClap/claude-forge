...
**3. The gate hook (project payload, not this plugin).** The full install's project `.claude/settings.json` wires a `PreToolUse` hook on `Bash|PowerShell` → `forge-gate-hook.cjs`, which enforces the three COMMAND hard gates (`destructive-delete`, `kill-by-name`, `git-destructive` — now including `git checkout .` / `-- <path>`, `git restore <path>`, `git switch -f`) with exit 2 and a plain NL/EN reason; deletes provably inside a scratch area pass. `permissions.deny` keeps `.env`, `.env.local`, the common `.env.*` variants and `secrets/**` out of the Read tool (`.env.example` stays readable). Never switch the hook off yourself to get past a block — ask the owner (setting `gate-hook`). This plugin itself ships no hooks.
