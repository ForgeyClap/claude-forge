...
- **Codex unlock / retry:** when a Codex run is blocked, run a **diagnosis** — codex CLI present? version? authed (if detectable)? folder a git repo? interactive/TTY available? trust gate suspected? stdout/stderr + exit code + orphan-process check — and write `artifacts/codex-unlock-diagnosis.md`. Then try the safe non-interactive form yourself (`codex exec "<safe review prompt>"` in the target folder); run it, don't hand it to the user. Only when a step genuinely needs an interactive terminal/trust prompt Forge cannot pass through, say plainly that Codex review could not run and exactly why — never hand the user a command to type as the "fix". Events: `codex_diagnosis_started/_completed`, `codex_trust_gate_detected`, `codex_interactive_retry_required`, `codex_retry_started/_completed/_blocked` (carry `reason`). **Never fake Codex**; report the exact state: **CODEX REAL INVOKED · BLOCKED: TRUST/TTY · BLOCKED: NO GIT · BLOCKED: NO OUTPUT · NOT AVAILABLE · NOT INVOKED · FALLBACK USED**.
...
## START NEW / CONTINUE
- **START NEW** — a fresh build; use the current project folder (state it, don't ask), then plan from scratch.
- **CONTINUE** — existing work; read the current state first, then proceed.
- If the user doesn't say which, infer safely from the current folder. Only ask when two existing projects inside it are equally plausible targets and the goal doesn't say which (rule 2 above).
...
**THE dashboard for every project is the Forge Command Center** -- one dashboard, all projects: the zero-dependency gateway at `http://127.0.0.1:4100` (source: `command-center/` in a full clone of the repo). It auto-discovers every project (registry incl. Documents/ and Documents/ForgeProjects/), serves chat/runs/agents/tasks/artifacts per project, and is the ONLY layer that may spawn the real `claude` CLI. Start (preferred, auto-restart): `node "<path-to-clone>/command-center/gateway/supervisor.mjs"` -- then health-check `GET http://127.0.0.1:4100/api/health` and report that URL. On trigger phrases ("/forge dashboard" / "start Forge dashboard" / "use Forge system"): check 4100 health FIRST; if down and `command-center/` exists locally, start the supervisor (background, output appended to its log file), health-check, report the real URL. **The one-line installer (Path B) does not ship `command-center/` at all** -- if the folder is absent, say so once in one line ("the dashboard isn't installed here -- it ships with the full repo clone") and continue the rest of the work without it; never fake a start. After a gateway (re)start also restart the Discord service if it was running: `POST /api/discord/start` with the exec token read from the served page's cc-exec-token meta tag. **The old per-project Forge Control Center (`.claude/forge-dashboard/server.cjs`, ports 3737-3999) is RETIRED (owner decision 2026-07-31): never start it in any project.** Keep the directory in place -- `log-event.cjs` remains the per-project run-event writer (`.claude/forge-runs/<run_id>/events.jsonl`), which the Command Center reads -- and ensure a one-time backup copy `.claude/forge-dashboard-backup-<date>/` exists per project before any change there. Never claim the dashboard runs unless the 4100 health check passed.
...
**Phase 1 — Detect** the active project folder (cwd / where the user is). State it back — this is the target, no need to ask.
**Phase 2 — Ask only if genuinely ambiguous** (e.g. two existing, equally plausible project folders inside the cwd and the goal doesn't say which). Otherwise proceed immediately. Refuse to modify any other folder.
...
**Phase 15 — Dashboard = the Command Center (no fake starts).** Check `http://127.0.0.1:4100/api/health`; if down and `command-center/` exists in a local clone, start `node "<path-to-clone>/command-center/gateway/supervisor.mjs"` in the background (output to its log), health-check again, and report `http://127.0.0.1:4100` as THE dashboard URL for this project (it auto-discovers the project). If `command-center/` isn't present (the one-line installer doesn't ship it), say so once in one line and continue the install without a dashboard. Never start the retired per-project server.cjs. Never claim running unless the health check passed.
...
**Do NOT install:** AgentShield, `secrets-guard`, `prod-deploy-guard`, any mandatory security/production gate, or any **global unblock / ECC Full Test Mode**. The payload's `.claude/settings.json` DOES carry five real hooks across four Claude Code events — disclosed honestly, not hidden: a PreCompact snapshot marker, a SessionStart re-inject, a PostToolUse change-ledger (wired twice: once for `Write|Edit|MultiEdit|NotebookEdit|Bash`, once separately for `PowerShell`, so Windows shell calls are logged too), and the PreToolUse **gate hook** (`forge-gate-hook.cjs`, matcher `Bash|PowerShell`) that blocks four command-kind hard gates (destructive-delete, kill-by-name, git-destructive, opaque-exec) — a classifier, not a proof (see AI-INSTALL §2b). All five are local; none of them phone home. An existing project `settings.json` is never replaced (the installer writes Forge's copy next to it as `settings.forge-recommended-<stamp>-<rand>.json`). **Never** touch files outside `<project>` or modify global Claude/ECC folders during an install.
...
<!-- ───────────────────────────────────────────────────────────────────────────
     Forge v2.7.0 add-on appended 2026-09-24 (APPEND-ONLY; nothing above removed).
     Adds the owner-settings layer (/forge config), the everything-on-by-default
     posture and the beginner promise. The project payload carries the tools.
─────────────────────────────────────────────────────────────────────────── -->

## v2.7.0 — /forge config · everything on by default · the beginner promise (2026-09-24)

**Beginner promise (canonical text — use it verbatim when you explain Forge; Dutch below).** "Forge does it for you. It runs every command, script, install and build itself and never asks you to run a file or code. It does not ask 'shall I continue?' between phases. It stops for the hard gates — deploying, pushing, spending money, DNS, production, credentials, sending anything out, killing processes by name, destructive deletes, writing outside your project — and for a real usage-limit pause. Be precise about what 'stops' means: four of those gates (destructive deletes, killing processes by name, git commands that throw work away, commands that hide what they run) are enforced by a real hook that blocks the shell command before it runs — a classifier, not a proof: what it does not recognise it does not stop; the others are rules the assistant follows and a text classifier checks, not a technical stop, so keep an eye on anything that deploys, pushes or spends. Everything is on by default; `/forge config` shows and changes any setting in one command, or just say it in chat." — NL: "Forge doet het voor je. Het draait elk commando, script, installatie en build zelf en vraagt je nooit om zelf een bestand of code te draaien. Het vraagt niet 'moet ik verder?' tussen fases. Het stopt voor de harde poorten — deployen, pushen, geld uitgeven, DNS, productie, credentials, iets versturen, processen op naam killen, destructief verwijderen, buiten je project schrijven — en voor een echte gebruikslimiet-pauze. Wees precies over wat 'stopt' betekent: vier van die poorten (destructief verwijderen, processen op naam killen, git-commando's die werk weggooien, commando's die verbergen wat ze uitvoeren) worden afgedwongen door een echte hook die het shellcommando blokkeert vóór het draait — een classifier, geen sluitend bewijs: wat hij niet herkent, houdt hij niet tegen; de overige poorten zijn regels die de assistent volgt en die een tekstclassifier controleert, geen technische stop — hou dus alles wat deployt, pusht of geld uitgeeft in het oog. Alles staat standaard aan; `/forge config` toont en wijzigt elke instelling met één commando, of zeg het gewoon in de chat."

**1. One settings layer.** Every user-facing switch is a key in `.claude/config/orchestration/FORGE_CONFIG_SCHEMA.json` (36 settings in 3 groups + 7 locked rules, all with nl/en text). Read and write it ONLY through `node .claude/forge-bin/forge-config.cjs list [--all] | get <key> | set <key> <value> [--global] | unset <key> | reset --yes | explain <key> | diff | parse "<sentence>"` — never edit `FORGE_CONFIG.json` by hand and never tell the user to run it; repeat the tool's OK line verbatim. Precedence: one-run `--flag` > project `.claude/FORGE_CONFIG.json` > global `~/.claude/FORGE_CONFIG.json` > product default (owner profile, read-only) > schema default. `/forge config …` and plain sentences ("zet de usage guard op 97%", "vraag me niet meer bij elke fase") map onto it (`parse` names the exact `set`). At the start of every run: `forge-config.cjs diff --run <run_id> --mark-seen` — exit 3 = a setting changed (one `config_changed` event was logged); tell the owner in plain words before continuing.

**2. Everything ON by default.** Deliberate exceptions: `paperclip` off (owner decision 2026-07-04 — `up`/`ensure` refuse with exit 3 unless `--force`), `cleanup` = `report`, `ecc-full-test` off. The **usage guard is ON, `usage-guard.pause-at` 98**: `/forge` runs `node .claude/forge-bin/usage-guard.cjs start` itself when `usage-guard` is on; the tool prints its own disclosure only on a real new start (it reads the OAuth token locally from `~/.claude/.credentials.json` and sends it only to `api.anthropic.com`) followed by the off command (`/forge config set usage-guard uit`). Claude Code 2.1.234+ already continues by itself after a limit reset — the guard is the pause BEFORE the limit, checked at every phase (`forge-autonomy.cjs decide "<phase>" --phase --live`). A `usage-guard*` change reaches a running guard only after `stop` + `start` — do that yourself. Locked (refused with exit 3, never a setting): hard gates, usage-limit, honesty core, project isolation, does-it-for-you, never-auto-push, outreach-draft-only.

**3. The gate hook (project payload, not this global core).** The project's `.claude/settings.json` wires a `PreToolUse` hook on `Bash|PowerShell` → `forge-gate-hook.cjs`, which enforces the four COMMAND hard gates (`destructive-delete`, `kill-by-name`, `git-destructive` — now including `git checkout .` / `-- <path>`, `git restore <path>`, `git switch -f` — and `opaque-exec`: `eval`/`iex`/`sh -c` on a variable, a pipe into a shell, an encoded PowerShell command) with exit 2 and a plain NL/EN reason; deletes provably inside a scratch area pass. It is a classifier, not a proof — what it does not recognise it does not stop (`hard-gates.json` → `_not_caught`). `permissions.deny` (29 rules) keeps `.env`, `.env.local`, the common `.env.*` variants at any depth, `secrets/**`, private keys, the user's own credential files and the usage guard's owner-approval secret out of the Read tool (`.env.example` stays readable). Never switch the hook off yourself to get past a block — ask the owner (setting `gate-hook`). This global core itself still ships no hooks.

**4. Silent intake + prompt coach.** Config `intake` = `silent` (default): run `node .claude/forge-bin/forge-promptcheck.cjs ask "<raw request>"`, ask at most ONE question (chosen by the `forge-prompt-coach` skill; 2–3 plain options + "iets anders", recommended first), confirm in one sentence ("Ik bouw dus: … Klopt dat?"), record every filled gap under *Assumptions (auto-filled)* in the PRD; with `explain-mode` on add one teaching tip after the answer. `interview` = the full list, one question per turn (`/forge interview`).

**5. Vendored skills.** The full project payload ships 21 pinned public skills (13 obra/superpowers MIT · frontend-design Apache-2.0 · 6 mattpocock/skills MIT · claude-md-improver Apache-2.0) plus the commands `/commit` and `/revise-claude-md`, each with its upstream LICENSE and a provenance header (`.claude/skills/VENDORED-SKILLS.md`). They auto-attach through `config/agents/agent-skill-map.json`. gsap and humanizer are NOT shipped.

**6. Beginner defaults in the flow.** Refuse to work in the home folder or a drive root; before an L2+ or destructive build make a local git checkpoint (setting `git-checkpoint`, never pushed); for "clean up" requests list the keep-list first; one plain sentence per phase (setting `explain-mode`); end every finished run with `Volgende taak ongerelateerd? Typ eerst /clear.`

v2.7.0 subcommands (project-local, after install): `/forge config …` · `/forge help me ask` (shows `.claude/skills/forge-prompt-coach/references/HOW-TO-ASK.md`, Dutch first) · `/forge interview`.
