...
For any BUILD/create/automate task (L2+), BEFORE emitting work packages, run the Prompt Master intake so the project's real goal is captured (owner directive 2026-07-13; see `forge-intake` skill + forge-core "PROMPT MASTER ALWAYS ON" §1). After classifying the domain (Step 1), run `node .claude/forge-bin/forge-intake.cjs --type <slug> --task "<task>" --run <run_id>` and answer the question list **yourself** from the mission text, the project scan and `.claude/.forge-setup.json` (record the answers in the PRD as *Assumptions (auto-filled)*; ask the owner at most ONE question, only when two existing targets are equally plausible — beginner-first, 2026-09-23; the full questionnaire is opt-in via `/forge interview`). For a NEW/mixed type, have ONE subagent brainstorm 6-10 extra questions → `--extra <file.json>`. Feed the answers into the PRD (`forge-prd`) and quote them in the Boss dispatch prompts. Skip intake only for trivial Q&A/status/one-line-fix turns; never build on assumed answers (state any assumption for a skipped required question).
