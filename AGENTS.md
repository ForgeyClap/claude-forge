...
| **verify-boss** | Comprehensive verification and graded proof — runs the doctor self-test suite (136 suites, 8,500+ assertions per installation), gates behind a contract-check before "done", and grades the actual result of every claim. | On completion of any major build, before reporting done; also spot-checks during a long session to catch drift early. | `opus` | Bash, PowerShell, Read, Grep, Glob |
